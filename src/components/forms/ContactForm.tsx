...
export function ContactForm() {
  ...
  const onSubmit = async (data: ContactFormValues) => {
    ...
    try {
      const subject = encodeURIComponent(`${data.projectType} inquiry from ${data.name}`);
      const body = encodeURIComponent(`Name: ${data.name}\nEmail: ${data.email}\nTopic: ${data.projectType}\n\n${data.message}`);
      window.location.href = `mailto:soumyadippal35@gmail.com?subject=${subject}&body=${body}`;
            ...
            <FormItem>
              ...
              <Select onValueChange={field.onChange} defaultValue={field.value}>
                ...
                <SelectContent className="bg-popover z-50">
                  <SelectItem value="internship" className="font-light">
                    Internship
                  </SelectItem>
